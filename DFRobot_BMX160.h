...
#define DEFAULT_GRAVITY_STANDARD 9.80665f
...
/** Power mode settings */
/* Accel power mode */
#define BMX160_ACCEL_SUSPEND_MODE 0b0001'0000
#define BMX160_ACCEL_NORMAL_MODE 0b0001'0001
#define BMX160_ACCEL_LOWPOWER_MODE 0b0001'0010

/* Gyro power mode */
#define BMX160_GYRO_SUSPEND_MODE 0b0001'0100
#define BMX160_GYRO_NORMAL_MODE 0b0001'0101
#define BMX160_GYRO_FASTSTARTUP_MODE 0b0001'0111

/* Magn power mode */
#define BMX160_MAGN_SUSPEND_MODE 0b0001'1000
#define BMX160_MAGN_NORMAL_MODE 0b0001'1001
#define BMX160_MAGN_LOWPOWER_MODE 0b0001'1010

/** Range settings */
/* Accel Range */
enum eAccelRange_t : uint8_t {
  eAccelRange_2G = 0b0011, // 1 LSB = 0.000061035mg
  eAccelRange_4G = 0b0101, // 1 LSB = 0.000122070mg
  eAccelRange_8G = 0b1000, // 1 LSB = 0.000244141mg
  eAccelRange_16G = 0b1100 // 1 LSB = 0.000488281mg
};
...
enum eGyroRange_t : uint8_t {
  eGyroRange_2000DPS = 0b000, // 1 LSB = 0.0610352dps
  eGyroRange_1000DPS = 0b001, // 1 LSB = 0.0304878dps
  eGyroRange_500DPS = 0b010,  // 1 LSB = 0.0152439dps
  eGyroRange_250DPS = 0b011,  // 1 LSB = 0.0076220dps
  eGyroRange_125DPS = 0b100   // 1 LSB = 0.0038110dps
};
...
typedef struct {
  ...
  uint16_t length; /**< While calling the API  "BMX160_get_fifo_data" , length stores number of bytes in FIFO to be read
                      (specified by user as input) and after execution of the API ,number of FIFO data bytes available
                      is provided as an output to user */
...
typedef struct {
  ...
  eBmx160AnySigMotionActiveInterruptState_t
      any_sig_sel;      /**< Hold active interrupts status for any and sig motion 0 - Any-motion enable, 1 - Sig-motion
                           enable,  -1 neither any-motion nor sig-motion selected */
  sBmx160Cfg_t magnCfg; /**< Structure to configure Magnetometer sensor */
  sBmx160Cfg_t prevMagnCfg;  /**< Structure to hold previous/old magn config parameters.This is used at driver level to
                                prevent overwriting of same data, hence user does not change it in the code */
  sBmx160Cfg_t accelCfg;     /**< Structure to configure Accel sensor */
  sBmx160Cfg_t prevAccelCfg; /**< Structure to hold previous/old accel config parameters.This is used at driver level to
                                prevent overwriting of same data, hence user does not change it in the code */
  sBmx160Cfg_t gyroCfg;      /**< Structure to configure Gyro sensor */
  sBmx160Cfg_t prevGyroCfg;  /**< Structure to hold previous/old gyro config parameters. This is used at driver level to
                                prevent overwriting of same data, hence user does not change it in the code */
...
typedef struct {
  ...
  uint16_t flatHoldTime : 2; /**< delay time for which the flat value must remain stable for the flat interrupt to be
                                generated */
...
public:
   ...
   * @param range
   * @n       eGyroRange_2000DPS     Gyroscope sensitivity at 2000dps (0b000, 1 LSB = 0.061dps)
   * @n       eGyroRange_1000DPS     Gyroscope sensitivity at 1000dps (0b001, 1 LSB = 0.031dps)
   * @n       eGyroRange_500DPS      Gyroscope sensitivity at 500dps (0b010, 1 LSB = 0.015dps)
   * @n       eGyroRange_250DPS      Gyroscope sensitivity at 250dps (0b011, 1 LSB = 0.0076dps)
   * @n       eGyroRange_125DPS      Gyroscope sensitivity at 125dps (0b100, 1 LSB = 0.0038dps)
   */
  void setGyroRange(eGyroRange_t range);
   ...
   */
  void setAccelRange(eAccelRange_t range);
...
private:
  ...
  /**
   * @fn _defaultParamSettg
   * @brief Setting default Parameters
   * @param dev
   */
  void _defaultParamSettg(sBmx160Dev_t *dev);
   ...
   */
  void _readReg(uint8_t reg, uint8_t *pBuf, uint16_t len);
   ...
   */
  void _writeReg(uint8_t reg, uint8_t *pBuf, uint16_t len);
   ...
   * @return return the actually written length
   */
  void _writeBmxReg(uint8_t reg, uint8_t value);
  ...
  sBmx160Dev_t *_bmx160;

  sBmx160SensorData_t *_magn;
  sBmx160SensorData_t *_gyro;
  sBmx160SensorData_t *_accel;
};
